...
`apex-plat-compose-mesh` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for compose analysis with deny and allow fixtures, explainable decision traces, and documented operating limits.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 3.
